 ...
 * @note   规则与 convert_utf8_to_unicode 一致(支持 1~3 字节序列)
 */
int convert_is_valid_utf8(const char *str)
{
    ...
    while (*p)
    {
        ...
        {
            /* 4 字节及以上、或孤立续字节, 视为非本模块可处理的 UTF-8 */
            return 0;
        }
    }
...
{
    if (str == NULL || gbk == NULL || gbk_size < 2)
    {
        return -1;
    }

    /* 合法 UTF-8(含纯 ASCII): 走转换表 */
    if (convert_is_valid_utf8(str))
    {
        if (convert_utf8_to_gbk(str, gbk) == 0)
        {
            return 0;
        }
        /* 转换缓冲不足等异常时退回原样拷贝 */
    ...
    /* 非法 UTF-8: 按 GBK/OEM 原样拷贝(FatFS 中文短文件名常见场景) */
    strncpy((char *)gbk, str, gbk_size - 1);
    gbk[gbk_size - 1] = 0;
    return 0;
}
