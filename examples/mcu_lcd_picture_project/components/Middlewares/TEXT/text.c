 ...
 * @note        该函数支持自动换行; 自动识别 UTF-8 / GBK 输入, 统一转 GBK 后显示
 * @param       x,y   : 起始坐标
 * @param       width : 显示区域宽度
 * @param       height: 显示区域高度
 * @param       str   : UTF-8 或 GBK 字符串
 * @param       size  : 字体大小,值：12,16,24
 * @param       mode  : 显示模式
 * @note                0, 正常显示(不需要显示的点,用LCD背景色填充,即g_back_color)
 * @note                1, 叠加显示(仅显示需要显示的点, 不需要显示的点, 不做处理)
...
void text_show_string(uint16_t x, uint16_t y, uint16_t width, uint16_t height, char *str, uint8_t size, uint8_t mode, uint32_t color, uint32_t g_back_color)
{
    ...
    /* 自动识别 UTF-8 / GBK, 统一为 GBK 后再描点 */
    if (convert_auto_to_gbk(str, gbk_buf, sizeof(gbk_buf)) != 0)
    {
        return;
    }
    pstr = gbk_buf;
    ...
    while ((*pstr != 0))
    {
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    lcd_show_char(x, y, *pstr, size, mode, color, g_back_color);
                }
