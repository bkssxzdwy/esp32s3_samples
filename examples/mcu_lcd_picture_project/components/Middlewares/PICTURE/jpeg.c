 ...
 */
uint16_t infunc(JDEC *decoder, uint8_t *buf, uint16_t len)
{
    uint16_t rb;                       /* 读取到的字节数 */
 ...
 * @retval      返回1输出成功
 */
uint16_t outfunc(JDEC *decoder, void *bitmap, JRECT *rect)
{
...
esp_err_t decode_jpeg(pixel_jpeg ***pixels, char *file, int screenWidth, int screenHeight, int *imageWidth, int *imageHeight)
{
    ...
    *pixels = malloc(sizeof(pixel_jpeg *) * screenHeight);
    ...
    if (f_jpeg == NULL)
    {
        ESP_LOGE(__FUNCTION__, "Cannot allocate FIL");
        ret = ESP_ERR_NO_MEM;
        goto err;
    }

    FRESULT fr = f_open(f_jpeg, (const TCHAR *)file, FA_READ);
    if (fr != FR_OK)
    {
        ESP_LOGW(__FUNCTION__, "Image open fail [%s] fr=%d", file, fr);
        free(f_jpeg);
        f_jpeg = NULL;
